...
void fastunpack(int at, ll h) {
   ull *wp = ht1+h*rwords ;
   for (int i=0; i<nunpack; i++)
      board[unpackdat[i].y][unpackdat[i].x] =
                LO + ((wp[unpackdat[i].off] >> unpackdat[i].shift) & lowmask) ;
}
...
//       showboard() ;
         ...
         int np = 0 ;
         for (int i=HI; i>=LO; i--) {
            if (at < firstvals.size() && i != firstvals[at]) {
               continue ;
            }
            int top = canwrite + 1 ;
            board[y][x] = i ;
            ...
            np = mnp - op[6] ;
            for (int d=0; np < 50 && oldpenalty + np < top && d<6; d++) {
